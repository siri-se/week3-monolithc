...
function createStatusButtons(taskId, currentStatus) {
    ...
    if (currentStatus === 'TODO') {
        buttons.push(`<button class="btn btn-warning btn-sm" onclick="updateTaskStatus(${taskId}, 'IN_PROGRESS')">→ In Progress</button>`);
        buttons.push(`<button class="btn btn-success btn-sm" onclick="updateTaskStatus(${taskId}, 'DONE')">→ Done</button>`);
    } else if (currentStatus === 'IN_PROGRESS') {
        buttons.push(`<button class="btn btn-secondary btn-sm" onclick="updateTaskStatus(${taskId}, 'TODO')">← To Do</button>`);
        buttons.push(`<button class="btn btn-success btn-sm" onclick="updateTaskStatus(${taskId}, 'DONE')">→ Done</button>`);
    } else if (currentStatus === 'DONE') {
        buttons.push(`<button class="btn btn-secondary btn-sm" onclick="updateTaskStatus(${taskId}, 'TODO')">← To Do</button>`);
        buttons.push(`<button class="btn btn-warning btn-sm" onclick="updateTaskStatus(${taskId}, 'IN_PROGRESS')">← In Progress</button>`);
    }
    buttons.push(`<button class="btn btn-danger btn-sm" onclick="deleteTask(${taskId})">🗑️ Delete</button>`);
    return buttons.join('');
}
