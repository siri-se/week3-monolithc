...
app.put('/api/tasks/:id', (req, res) => {
    ...
    values.push(id); 
     const sql = `UPDATE tasks SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
...
app.patch('/api/tasks/:id/status', (req, res) => {
    ...
    if (!validStatuses.includes(status)) return res.status(400).json({ error: 'Invalid status. Must be TODO, IN_PROGRESS, or DONE' });
